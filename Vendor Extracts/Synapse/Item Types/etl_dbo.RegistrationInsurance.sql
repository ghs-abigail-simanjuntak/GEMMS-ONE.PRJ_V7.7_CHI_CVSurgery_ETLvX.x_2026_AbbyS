...
SELECT
    ...
    CONCAT(ISNULL(CAST(i.PatientID AS varchar(50)), ''),'|',ISNULL(CAST(i.INUM AS varchar(50)), '')) AS [ExternalDataId],
    i.[PatientId] AS [ItemExternalDataId],
    '1' AS [ItemExternalDataVersion],
    CONCAT(i.[PatientID], '|', COALESCE(i.[INUM], ''), '|', COALESCE(i.[INSCODE], ''), '|', COALESCE(i.[POLICYNO], ''))  AS [InsuranceExternalDataId],
    CASE i.[INUM]
        WHEN 2 THEN 'Primary'
        WHEN 3 THEN 'Secondary'
        WHEN 4 THEN 'Tertiary'
        WHEN 5 THEN 'Quaternary'
        WHEN 6 THEN 'Quinary'
        WHEN 7 THEN 'Senary'
        ELSE 'Other'
    END AS [Priority],
